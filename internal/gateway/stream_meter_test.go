...
// noDone is a no-op onDone callback used in tests that don't inspect it.
...
func TestStreamMeterPassesThroughDataUnchanged(t *testing.T) {
	...
	sm := newStreamMeter(sseBody(input), "gpt-4o", 0, noDone)
...
func TestStreamMeterPicksUpModelFromChunk(t *testing.T) {
	...
	var gotModel string
	sm := newStreamMeter(sseBody(input), "initial-model", 0, func(model string, _, _, _ int, _ bool) {
		gotModel = model
	})
	_, _ = drain(sm)
...
func TestStreamMeterFallsBackToInitialModelWhenNoneInStream(t *testing.T) {
	...
	var gotModel string
	sm := newStreamMeter(sseBody(input), "fallback-model", 0, func(model string, _, _, _ int, _ bool) {
		gotModel = model
	})
	_, _ = drain(sm)
...
func TestStreamMeterCollectsTokensFromUsageChunk(t *testing.T) {
	...
	sm := newStreamMeter(sseBody(input), "gpt-4o", 0, func(_ string, p, c, tt int, _ bool) {
...
func TestStreamMeterIgnoresChunkWithZeroTotal(t *testing.T) {
	...
	var total int
	sm := newStreamMeter(sseBody(input), "m", 0, func(_ string, _, _, tt int, _ bool) {
		total = tt
	})
	_, _ = drain(sm)
...
func TestStreamMeterOnDoneFiredOnEOF(t *testing.T) {
	...
	var called int32
	sm := newStreamMeter(sseBody(input), "m", 0, func(string, int, int, int, bool) {
...
func TestStreamMeterOnDoneFiredOnClose(t *testing.T) {
	...
	var called int32
	sm := newStreamMeter(io.NopCloser(pr), "m", 0, func(string, int, int, int, bool) {
		atomic.AddInt32(&called, 1)
	})
...
func TestStreamMeterHandlesPartialReads(t *testing.T) {
	...
	sm := newStreamMeter(sseBody(input), "m", 0, func(_ string, p, _, tt int, _ bool) {
		prompt, total = p, tt
	})
...
// ---------------------------------------------------------------------------
// Fallback estimation (no usage chunk in stream)
// ---------------------------------------------------------------------------

// TestStreamMeterFallback_NoUsageChunk: when the stream ends without any
// usage chunk onDone must be called with estimated=true and non-zero counts
// derived from accumulated content length and the supplied prompt estimate.
func TestStreamMeterFallback_NoUsageChunk(t *testing.T) {
	// "Hello world" = 11 bytes → completion = 11/4 = 2
	...
	sm := newStreamMeter(sseBody(input), "gpt-4o", 20, func(_ string, p, c, tt int, est bool) {
	...
	if gotPrompt != 20 {
		t.Errorf("prompt: got %d, want 20 (promptEstimate)", gotPrompt)
	}
	// 11 bytes / 4 = 2
	if gotCompletion != 2 {
	...
	if gotTotal != 22 {
		t.Errorf("total: got %d, want 22 (20+2)", gotTotal)
	}
}

// TestStreamMeterFallback_WithUsageChunk: when a usage chunk is present the
// real token counts are used and estimated must be false.
func TestStreamMeterFallback_WithUsageChunk(t *testing.T) {
	input := sseChunk(textLine) + sseChunk(finishLine) + "data: [DONE]\n\n"

	var gotPrompt, gotCompletion, gotTotal int
	var gotEstimated bool
	sm := newStreamMeter(sseBody(input), "m", 100, func(_ string, p, c, tt int, est bool) {
		gotPrompt, gotCompletion, gotTotal = p, c, tt
		gotEstimated = est
	})
	_, _ = drain(sm)
